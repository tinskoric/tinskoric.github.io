...
```{r trendlines}
income_ordered_trends_static <- ggplot(all_state_counties_income, aes(reorder(County, Median_Household_Income, sum), Bachelor_or_Higher, color = Rural_Condition, label1 = Rural_Condition, label2 = Median_Household_Income, label3 = Bachelor_or_Higher)) + geom_point() +
...
income_ordered_trends <- ggplotly(income_ordered_trends_static, height = 416, width = 1248, tooltip = c("label1", "label2", "label3")) %>%
      layout(title = list(text = paste0("Bachelor’s Degree or Higher Attainment in Across 12 States by County", '<br>', '<sup>', "Data from 2017 to 2021", '</sup>'))) %>%
