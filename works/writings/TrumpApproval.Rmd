...
obama_trump_static <- ggplot(Both_Approval_Monthly, aes(ID, group = 12)) + 
...
obama_trump <- ggplotly(obama_trump_static, height = 416, width = 1248) %>%
      layout(title = list(text = paste0("Obama v. Trump Approval Polling", '<br>', '<sup>', "Approval Rating Aggregated by Month for Each Year of Presidency", '</sup>'))) %>%
      layout(annotations = list(x = 1, y = -0.1, 
text = "Source: Roper Center for Public Opinion Research at Cornell University", 
...
```{r ApprovalRegression}

regression_plot_static <- ggplot(Linear_Model_Prediction_vs_Actual, aes(ID, group = 12)) + 
...
regression_plot <- ggplotly(regression_plot_static, height = 416, width = 1248) %>%
      layout(title = list(text = paste0("Trump's Approval Rating Real v. Predicted", '<br>', '<sup>', "Approval Rating Aggregated by Month from 2017 to 2021 v. Predicted Approval (LOESS Regression)", '</sup>'))) %>%
      layout(annotations = list(x = 1, y = -0.1, 
text = "Source: Roper Center for Public Opinion Research at Cornell University", 
      showarrow = F, xref='paper', yref='paper', xanchor='right', yanchor='auto', xshift = 0, yshift = 0, font=list(size=8, color="white")))
